...
var login = function login(options) {
  console.log("小程序增强SDK-login流程,options:", options);
  ...
  var doLogin = () => getWxLoginResult(function (wxLoginError, wxLoginResult) {
    console.log("小程序增强SDK-获取Code");
    if (wxLoginError) {
      console.log("wxLoginError", wxLoginError);
    ...
    wx.request({
      ...
      success: function (result) {
        console.log("请求服务器登录地址，获得会话信息,result", result);
        ...
        if (data && data.code === 0 && data.data.skey) {
          ...
          if (res.userinfo) {
            ...
            console.log("存储Session信息");
            var session = {"skey":res.skey} 
            session.userinfo = res.userinfo;
            console.log("session", session);
            Session.set(session);
            options.success(userInfo);
            console.log("成功地响应会话信息,options:", options);
          } else {
            console.log('登录失败');
          ...
          options.fail(noSessionError);
          console.log("没有正确响应会话信息,options:", options);
        }
      },
